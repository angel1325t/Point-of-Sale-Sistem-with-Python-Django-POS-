...
function updateTransferSummary() {
  ...
  if (saleItems.length === 0) {
    ...
    btnNext?.classList.add('cursor-not-allowed', 'bg-blue-500');
    btnNext?.classList.remove('bg-blue-800');
    btnNext.disabled = true;
    montoInput.value = "";
    productsInput.value = "";
  } else {
    btnNext?.classList.remove('cursor-not-allowed', 'bg-blue-500');
    btnNext?.classList.add('bg-blue-800');
